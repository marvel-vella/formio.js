...
export default class SelectProductComponent extends Field {
    ...
    static get builderInfo() {
        return {
            ...
            schema: SelectProductComponent.schema()
        };
    }
    ...
    get defaultSchema() {
        return SelectProductComponent.schema();
    }
    ...
    itemValue(data, forceUseValue = false) {
        try {
            if (_.isObject(data)) {
                if (this.valueProperty) {
                    //console.log(`SelectProduct.itemValue: ${this.valueProperty}`);

                    if (!this.valueProperty.includes('{{')) {
                        return _.get(data, this.valueProperty);
                    }
                    else if (this.valueProperty === '{{site_product_id}}_{{site_product_server_id}}') {
                        return `${_.get(data, 'site_product_id')}_${_.get(data, 'site_product_server_id')}`;
                    }
                    else {
                        var result = data.value;
                        //we need to evaluate - the value contains variables in this format: {{variable_id}}_{{variable_server_id}}
                        var regex = /{{[a-zA-Z0-9_]+}}/g;
                        var found = this.valueProperty.match(regex);
                        if (found != null) {
                            //console.log(`SelectProduct.itemValue. Found: ${JSON.stringify(found)}`);

                            //console.log(`SelectProduct.itemValue. found.length: ${found.length}`);
                            result = this.valueProperty;
                            for (var i = 0; i < found.length; i++) {
                                var item = found[i];
                                console.log(`SelectProduct.itemValue. item: ${item}`);
                                var variable = item.replace('{{', '');
                                variable = variable.replace('}}', '');
                                //console.log(`SelectProduct.itemValue. variable: ${variable}`);

                                var dataValue = _.get(data, variable);
                                //console.log(`SelectProduct.itemValue. dataValue: ${dataValue}`);
                                result = result.replace(item, dataValue);
                                //console.log(`SelectProduct.itemValue. result: ${result}`);
                            }
                        }
                        return result;
                    }
